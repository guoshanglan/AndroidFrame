...
import com.zrlib.matisse.ui.widget.MediaGrid;
...
public class AlbumMediaAdapter extends
    ...
    public AlbumMediaAdapter(Context context, SelectedItemCollection selectedCollection, RecyclerView recyclerView) {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(new int[]{R.attr.matisse_item_placeholder});
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CAPTURE) {
            ...
            MediaViewHolder viewHolder = new MediaViewHolder(v);
            if (mSelectionSpec.cutter || mSelectionSpec.isSingleSelect()) {   //如果有裁剪或者只是单选，那么不需要展示选中框
                viewHolder.mMediaGrid.setCheckVisibility(View.GONE);
            }
            return viewHolder;
        }
        return null;
...
//            Drawable[] drawables = captureViewHolder.mHint.getCompoundDrawables();
            TypedArray ta = holder.itemView.getContext().getTheme().obtainStyledAttributes(new int[]{R.attr.matisse_capture_textColor});
...
//            captureViewHolder.mHint.setCompoundDrawables(drawables[0], drawables[1], drawables[2], drawables[3]);
    ...
    public void onThumbnailClicked(ImageView thumbnail, Item item, RecyclerView.ViewHolder holder) {
        if (mSelectionSpec.cutter) {
            if (mOnMediaClickListener != null && assertAddSelection(holder.itemView.getContext(), item)) {
                mOnMediaClickListener.onMediaClick(null, item, holder.getAdapterPosition());
            }
        } else if (mSelectionSpec.showPreview || mSelectionSpec.isSingleSelect()) {
