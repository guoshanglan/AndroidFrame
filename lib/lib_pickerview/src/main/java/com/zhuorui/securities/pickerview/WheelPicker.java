...
public class WheelPicker<T> extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsShowCurtainBorder) {
            ...
            float oldw = mPaint.getStrokeWidth();
            float w = getResources().getDisplayMetrics().density * 0.5f * 0.5F;
            w = w < 1 ? 1 : w;
...
//			canvas.drawRect(mDrawnRect, mPaint);
    ...
    public void setSelectedItemTextStyle(Typeface typeface) {
        mSelectedItemPaint.setTypeface(typeface);
        postInvalidate();
    }
